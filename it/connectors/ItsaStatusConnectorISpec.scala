  ...
  "ItsaStatusConnector" should {
    ...
    "return an exception" when {
      "the status determination service returns OK and an invalid JSON response body" in {
        ...
        result.futureValue shouldBe Left(ErrorModel(OK, "Invalid Json for itsaStatusResponseHttpReads: List((,List(JsonValidationError(List(error.expected.jsarray),WrappedArray()))))"))
      }
    }
