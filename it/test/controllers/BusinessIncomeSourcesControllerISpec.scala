...
class BusinessIncomeSourcesControllerISpec extends ComponentSpecBase {
    ...
    s"return a $NO_CONTENT response" when {
      "income sources are successfully submitted" in {
        Seq(false, true).foreach { withIdempotency =>
          ...
          CreateIncomeSourceStub.stubItsaIncomeSource(Json.toJson(testCreateIncomeSources(withIdempotency))(CreateIncomeSourcesModel.hipWrites(testMtdbsaRef))
          )(CREATED, testCreateIncomeSuccessBody)

          val result: WSResponse = IncomeTaxSubscription.businessIncomeSource(testMtdbsaRef, testCreateIncomeSourcesJson(withIdempotency))
  ...
  def testCreateIncomeSourcesJson(withIdempotency: Boolean = false): JsValue = {
