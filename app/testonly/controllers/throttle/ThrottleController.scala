...
class ThrottleController @Inject()(override val messagesApi: MessagesApi,
      ...
      req.getQueryString("throttle") match {
        case Some(throttleId) =>
          ...
          throttleValMaybe match {
            case Some(throttleVal) => Ok(throttleKey + " is set to " + throttleVal)
            case _ => BadRequest(s"Throttle id $throttleId has no configured value")
          }
        case _ => BadRequest(s"No throttle id provided")
