...
class ExtractorsSpec extends CommonSpec with Extractors {
  ...
  "getArnFromEnrolments" when {
    "called with no enrolments present" should {
      "return none" in {
        getArnFromEnrolments(Enrolments(Set.empty)) shouldBe None
      }
    }
    "called with no value present in an enrolment" should {
      "return some(value)" in {
        ...
        getArnFromEnrolments(noEnrolments) shouldBe None
      }
    }
    "called with an appropriate value present in an enrolment" should {
      "return some(value)" in {
        ...
        getArnFromEnrolments(oneEnrolment) shouldBe Some("idValue")
      }
    }
    "called with several appropriate values present" should {
      "return the first value" in {
        ...
        getArnFromEnrolments(oneEnrolment) shouldBe Some("idValue1")
      }
    }
